...
const isNotificationSupported = () => {
  try {
    // expo-notifications requires native modules not available in Expo Go
    return Device.isDevice && Notifications.getExpoPushTokenAsync !== undefined;
  } catch (error) {
    console.log('Notifications not supported in this environment (Expo Go)');
    return false;
  }
};
...
export async function registerForPushNotifications(): Promise<string | null> {
  try {
    ...
    if (!isNotificationSupported()) {
      console.log('⚠️ Push notifications not available in Expo Go. Build with EAS to test notifications.');
      return null;
    }
